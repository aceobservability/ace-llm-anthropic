...
func TestChat_StreamMapsToOpenAISSE(t *testing.T) {
...
func TestChat_NonStreamMapsToOpenAIJSON(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	p, err := New(llm.LLMConfig{BaseURL: server.URL, APIKey: "sk-ant-test"})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
